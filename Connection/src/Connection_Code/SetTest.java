 ...
 * 要求: 向Set中添加的元素，其所在的类一定要重写hashCode()方法和equals()方法
 * 要求: 重写的hashCode()方法和equals()方法尽可能保持一致性: 相等的对象必须具有相同的散列码。
 *      重写两个方法的小技巧: 对象中用作equals()方法比较的Field，都应该用来计算hashCode
 * @author:superherozhang
 * @create:2022-03-13 15:46
 */
public class SetTest {
    ...
    *
    * 2.不可重复性：保证添加的元素，按照equals()方法判断时，不能返回true。即:相同的元素只能添加一个
    *
    * 二、添加元素的过程: 以HashSet为例:
    *     向HashSet中添加元素a，首先调用元素a所在类的hashCode()方法，计算元素a的哈希值。
    *     此哈希值接着通过某种算法计算出在HashSet底层数组中的存储位置(即为：索引位置)，判断数组此位置上是否已经有元素。
    ...
    * jdk 7: 元素a存放在数组中，指向原来的元素
    * jdk 8: 原来的元素存放在数组中，指向元素a
    *   总结:七上八下
    *
